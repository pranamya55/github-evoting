...
/**
 * Implements the GetCorrectnessInformation algorithm.
 *
 ...
 * @param {ImmutableArray<string>} actualVotingOptions - the list of actual voting options of size m'. Must be non-null and a subset of the pTable's actual voting options.
 * All actual voting options must be distinct and valid xs:token.
 *
 * @returns {ImmutableArray<string>} - the list of correctness information.
 */
export function getCorrectnessInformation(primesMappingTable: PrimesMappingTable, actualVotingOptions: ImmutableArray<string>): ImmutableArray<string> {
	...
	if (m_prime === 0) {
		// m <- n
		return ImmutableArray.from(pTable.elements.map(entry => entry.correctnessInformation));
	} else {
		// m <- m'
		return v_prime_vector
			.map(actualVotingOption => primesMappingTable.getPrimesMappingTableEntry(actualVotingOption))
			.map(pTableEntry => pTableEntry.correctnessInformation);
	}
}
